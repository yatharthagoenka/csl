...
class FuncDefNode:
    def __init__(self,var_name_tok,arg_name_tok,body_node,should_auto_return):
        ...
        self.arg_name_tok = arg_name_tok    # Function Arguments
        self.body_node = body_node          # Function Body
...
class ParseResult:
    ...
    def register(self,res):
        self.last_registered_adv_count = res.adv_count
...
class Parser:
    ...
    def statements(self):
        ...
        while self.cur_tok.type == TT_NEWLINE:
            res.register_adv()
            self.advance()
        ...
        while True:
            newline_count = 0
            while self.cur_tok.type == TT_NEWLINE:    # Skip all newlines
                res.register_adv()
                self.advance()
                newline_count += 1
            ...
            if not statement:
                self.reverse(res.to_reverse_count)      # If there werent any statements, self.exp wouldve advanced a few times, which needs to be reversed before parsing ahead
                more_statements = False
                continue
            statements.append(statement)
        
        return res.success(ListNode(
            statements, pos_start, self.cur_tok.pos_end.copy()
        ))
    ...
    def statement(self):
        ...
        if self.cur_tok.matches(TT_KEYWORD, 'return'):
            res.register_adv()
            self.advance()

            expr = res.try_register(self.exp())
            if not expr:
                self.reverse(res.to_reverse_count)
            return res.success(ReturnNode(expr, pos_start, self.cur_tok.pos_start.copy()))

        if self.cur_tok.matches(TT_KEYWORD, 'continue'):
            res.register_adv()
            self.advance()

            return res.success(ContinueNode(pos_start, self.cur_tok.pos_start.copy()))

        if self.cur_tok.matches(TT_KEYWORD, 'break'):
            res.register_adv()
            self.advance()

            return res.success(BreakNode( pos_start, self.cur_tok.pos_start.copy()))

        expr = res.register(self.exp())
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.cur_tok.pos_start,self.cur_tok.pos_end,
                "Expected 'return', 'break', 'continue', 'let',INT, FLOAT, IDENTIFIER, '[', '+', '-' or '(', '['\n"
            ))
    ...
    def call(self):
        ...
        if self.cur_tok.type == TT_LPAR:
            res.register_adv()
            self.advance()
            arg_nodes = []

            if self.cur_tok.type == TT_RPAR:     # Meaning there arent any more args
                res.register_adv()
                self.advance()
            else:
                arg_nodes.append(res.register(self.exp()))
                if res.error:
                    return res.failure(InvalidSyntaxError(
                        self.cur_tok.pos_start, self.cur_tok.pos_end,
                        "Expected ')', 'let', for, while, fnc, INT, FLOAT, IDENTIFIER, '+', '-','[' or '('\n"
                    ))
    ...
    def else_expr(self):
        ...
        if self.cur_tok.matches(TT_KEYWORD, 'else'):
            res.register_adv()
            self.advance()

            if self.cur_tok.type == TT_NEWLINE:
                res.register_adv()
                self.advance()
                ...
                if self.cur_tok.matches(TT_KEYWORD, 'end'):
                    res.register_adv()
                    self.advance()
                else:
                    return res.failure(InvalidSyntaxError(
    ...
    def elif_or_else_expr(self):
        ...
        if self.cur_tok.matches(TT_KEYWORD,'elif'):
    ...
    # case_keyword: if or elif
    def if_expr_cases(self,case_keyword):
        res = ParseResult()
        cases = []
        else_case = None

        if not self.cur_tok.matches(TT_KEYWORD, case_keyword):
            return res.failure(InvalidSyntaxError(
                self.cur_tok.pos_start, self.cur_tok.pos_end,
        ...
        condition = res.register(self.exp())
        if res.error: return res

        if not self.cur_tok.matches(TT_KEYWORD, 'then'):
            return res.failure(InvalidSyntaxError(
                self.cur_tok.pos_start, self.cur_tok.pos_end,
                "Expected 'then'\n"
            ))
    ...
    def for_expr(self):
        ...
        if not self.cur_tok.matches(TT_KEYWORD,'for'):
            return res.failure(InvalidSyntaxError(
                self.cur_tok.pos_start,self.cur_tok.pos_end,
                f"Expected 'for'\n"
            ))
        ...
        start_value = res.register(self.exp())
        if res.error: return res

        if not self.cur_tok.matches(TT_KEYWORD,'to'):
            return res.failure(InvalidSyntaxError(
                self.cur_tok.pos_start,self.cur_tok.pos_end,
                "Expected 'to'\n"
            ))
        ...
        end_value = res.register(self.exp())
        if res.error: return res

        if self.cur_tok.matches(TT_KEYWORD,'inc'):
            res.register_adv()
            self.advance()

            inc_value = res.register(self.exp())
            if res.error: return res
        else:
            inc_value = None
        ...
        if self.cur_tok.type == TT_NEWLINE:
            ...
            if not self.cur_tok.matches(TT_KEYWORD,'end'):
                return res.failure(InvalidSyntaxError(
                    self.cur_tok.pos_start, self.cur_tok.pos_end,
                    "Expected 'end' \n"
                ))
    ...
    def while_expr(self):
        ...
        if not self.cur_tok.matches(TT_KEYWORD,'while'):
            return res.failure(InvalidSyntaxError(
                self.cur_tok.pos_start,self.cur_tok.pos_end,
                "Expected 'while'\n"
            ))
    ...
    def func_def(self):
        ...
        if not self.cur_tok.matches(TT_KEYWORD,'fnc'):
            return res.failure(InvalidSyntaxError(
                self.cur_tok.pos_start,self.cur_tok.pos_end,
                "Expected 'fnc'\n"
            ))
        ...
        if self.cur_tok.type == TT_IDENTIFIER:
            arg_name_tok.append(self.cur_tok)
            res.register_adv()
            self.advance()
        ...
        if self.cur_tok.type == TT_ARROW:
            ...
            node_to_return = res.register(self.exp())
            if res.error: return res

            return res.success(FuncDefNode(var_name_tok,arg_name_tok,node_to_return,True))
    ...
    def list_expr(self):
        ...
        if self.cur_tok.type == TT_RSQ:
            res.register_adv()
            self.advance()
        else:
            element_nodes.append(res.register(self.exp()))
            if res.error:
                return res.failure(InvalidSyntaxError(
                    self.cur_tok.pos_start, self.cur_tok.pos_end,
                    "Expected ']', 'let', for, while, fnc, INT, FLOAT, IDENTIFIER, '+', '-','[' or '('\n"
...
class Number(Value):
    ...
    # Since we've got multiple types, it wouldnt make sense to add numbers to function, therefore the else statements in each
    ...
    def check_args(self, arg_names, args):
        ...
        if len(args) > len(arg_names):
            return res.failure(RTError(
                ...
                f"{len(args)-len(arg_names)} added args passed into '{self.name}' \n",
                self.context
            ))
        ...
        if len(args) < len(arg_names):
            return res.failure(RTError(
                self.pos_start,self.pos_end,
                f"{len(arg_names)-len(args)} less args passed into '{self.name}' \n",
                self.context
            ))
...
class Function(BaseFunction):
    ...
    def execute(self,args):
        ...
        # New context for every function, and a table which stores its values while it runss
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_input_int(self, exec_ctx):
        ...
        try:
            ...
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                f"'{text}' must be an integer. Try again!",
                exec_ctx
            ))
    ...
    def execute_append(self, exec_ctx):
        llist = exec_ctx.symbol_table.get("list")
    ...
    def execute_pop(self, exec_ctx):
        ...
        try:
            elem = llist.elements.pop(index.value)
            return RTResult().success(elem)
        except:
            return RTResult().failure( RTError(
               self.pos_start, self.pos_end,
               "Index out of bounds",
...
class Interpreter:
    ...
    def visit_FuncDefNode(self,node,context):
        ...
        # Storing func name (IF it exists) for calling it 
